...
7. **Keep the sitemap current.** This site does NOT use `@astrojs/sitemap`; it serves hand-rolled Yoast-style endpoints driven by `src/lib/sitemap.ts`. When you add a page, append its route to the `PAGES` array (or, for a post, it flows in automatically through `getPublishedPosts()`) with today's `lastmod`, and set the page's schema `dateModified` to match. Then re-submit `sitemap-index.xml` in Google Search Console.
8. Run through the [Visual audit + ship checklist](#visual-audit--ship-checklist) before declaring done.
...
11. **Sitemap + freshness, every change (not launch-only).** If you added, edited, or relaunched the page, bump its `lastmod` in `src/lib/sitemap.ts` (the `PAGES` array; posts carry their own `<lastmod>` via `getPublishedPosts()`) and bump the page's schema `dateModified` (and any visible "Updated" date) to the same date. A stale `dateModified` is what makes Google print an old date in the SERP and suppresses re-crawling. Then re-submit the sitemap in Google Search Console (`mcp__gsc__submit_sitemap` on `sitemap-index.xml`) and confirm it processed. Two gotchas: GSC can hold STALE sitemap records from the old Yoast/WP era, so a re-submit forces re-processing; and a sitemap INDEX reading "0 pages discovered" is NORMAL (the index has no URLs of its own, the `page-sitemap.xml` / `post-sitemap.xml` children carry them), so check the children rather than redoing the sitemap.
...
> **Sitemap currency is an every-change job, not a launch-only one.** This project replaced `@astrojs/sitemap` with hand-rolled Yoast-style endpoints (`src/pages/sitemap-index.xml.ts` fanning out to `page-sitemap.xml.ts` + `post-sitemap.xml.ts`), all driven by `src/lib/sitemap.ts`. The static `PAGES` array carries an explicit `<lastmod>` per route; posts pull `<lastmod>` from `getPublishedPosts()`. Whenever you add, edit, relaunch, or meaningfully change any page or post, bump that page's `lastmod` (and its schema `dateModified`) to the change date, then re-submit the sitemap in Search Console. A stale `lastmod` suppresses re-crawling and makes fresh content look old. (Where the older template text below still mentions `@astrojs/sitemap` or `dist/sitemap-index.xml` from `npm run build`, treat it as superseded by the hand-rolled setup; see the 2026-06-22 Decision-log row.)
...
| 2026-06-17 | JSON-LD — kill duplicate "Local business" listings (homepage `personSchema` + global node) | **Two changes (Josh's call, "reconcile sitewide to match live"):** (1) Homepage [index.astro]: removed `workLocation: offices.map(officePlace)` + the now-unused `officePlace` helper from the Person schema. (2) Sitewide [Layout.astro]: retyped the global `orgSchema` from `LocalBusiness` → `Organization`. | Google's Rich Results "Local businesses" report counted **8** business items on the homepage where live emits **3**. The 5 extras were rebuild-only: 4 office `Place` nodes (from Person.`workLocation`) + 1 global `LocalBusiness`. Live homepage has **no Person node and no global business node** — its only local-business schema is the standalone `FinancialService` office graph. Fix (1) drops the 4 phantom Place listings (Person node kept as E-E-A-T, just no longer spawns Places). Fix (2) makes the global publisher entity an `Organization` (not a LocalBusiness subtype → not counted), keeping its `@id` so `WebSite.publisher` + `Person.worksFor` references stay valid. **Net: 8 → 3, matching live.** Safe sitewide: 22/26 pages carry their own `FinancialService` (local-business eligibility unaffected); the 4 without it (sitemap/privacy/blog/404) shouldn't be local-business results anyway. ⚠️ `npm run validate` NOT run this session (npm errored in-env: "Exit handler never called!", `node_modules` absent) — run before deploy + re-test in Rich Results. |
| 2026-06-22 | Sitemap — Yoast-style split (replaces `@astrojs/sitemap`) | **Removed the `@astrojs/sitemap` integration** (it emits one combined `sitemap-index.xml` + `sitemap-0.xml`) and **hand-rolled three endpoints** to reproduce the live Yoast shape: `src/pages/sitemap-index.xml.ts` (index → fans out to the two children), `src/pages/post-sitemap.xml.ts` (32 blog posts, from `getPublishedPosts()`, preserved `/<category>/<slug>/` permalinks, `<lastmod>` = `modifiedDate ?? pubDate`), `src/pages/page-sitemap.xml.ts` (25 static pages). Shared logic + the static-page list w/ preserved live lastmods in `src/lib/sitemap.ts`. Dropped the now-obsolete `/page-sitemap.xml` + `/post-sitemap.xml` → index 301s from `_redirects` (they're real routes now); kept `/sitemap.xml` + `/sitemap_index.xml` aliases. `@astrojs/sitemap` uninstalled. | Live WP (Yoast) served a sitemap INDEX fanning out to per-type child sitemaps (`post-sitemap.xml` + `page-sitemap.xml`), and that's the structure Google already crawled. Verified parity: index `<lastmod>`s match live exactly (post `…23:15:42`, page `…23:13:19`); 25 pages + 32 posts match live counts. EXCLUDES (to match live page-sitemap): `/privacy-policy/` (Yoast-noindex), `/[category]/` archives (live had no category-sitemap), paginated `/blog/page/N/` (preserves the prior `@astrojs/sitemap` filter behavior). `npm run validate` clean (66 pages). Image `<image:image>` entries from the live sitemaps were intentionally omitted (optional; would reference WP upload paths). |
| 2026-06-18 | VideoObject schema on every page/post with a video (Josh request, post-launch) | Added a `videoId?` prop to `Layout.astro` that emits a Google-valid **VideoObject** JSON-LD node. Wired into the **11 pages** with lite-youtube embeds (home, what-is-a-cbi, + 9 city pages) and **11 blog posts** with Phil's own YouTube embeds (new optional `videoId` frontmatter field, validated in `content.config.ts`). Real metadata table in **`src/lib/videos.ts`**. **EXCLUDED** the third-party "Valcon General" video (`OrkFtQ6Jhgs`) in the tenant-improvement guest post — not Phil's content, don't claim schema for it. Verified: 22 dist pages carry a valid VideoObject, 0 JSON parse errors, Valcon post = 0. | Josh asked for video schema after launch to make the clips eligible for video rich results. ⚠️ **`name` / `uploadDate` / `duration` in `videos.ts` are REAL values pulled from each video's YouTube watch page (@Philsellsbiz) — never fabricate `uploadDate` (invalid schema = GSC error). Re-pull if a video is swapped.** Descriptions are trimmed faithful summaries with the originals' dated stat-claims omitted on purpose. Modest lever (YouTube often wins the rich result for embedded video), but cheap + correct. |
| 2026-06-25 | Onsite fact-fix + internal-linking batch (branch `onsite/factfix-internal-links`) | **Copy fixes:** home "BBB Member A+Rated" → "BBB Accredited, A+ Rated"; Tempe "the the largest" → "the largest" (in `_tempe-business-broker-content.html`); Peoria `mapEmbed` re-centered Fountain Hills → Peoria, AZ. **Internal linking:** added contextual in-body links from ~30 blog posts up to `/phoenix-`, `/mesa-`, `/scottsdale-business-broker/` + the seller/buyer/CBI/listings hubs (2-4 per post, descriptive anchors); added Phoenix + Scottsdale to the `business-sellers` + `business-buyers` "areas we serve" lists. | From the 2026-06-25 website fact-check + internal-linking audit (the 3 money pages had ~0 in-body inbound links; blog = the largest link pool). Approved by Josh. **HELD:** the Utopia "E vs W" address — the site has two conflicting map embeds (east/85024 vs west/85027), so it needs Phil/GBP confirmation before any NAP change. **NOT touched:** the self-rated `AggregateRating` (kept per the 2026-06-15 decision — do not strip). |
| 2026-06-26 | Sitemap maintenance + `llms.txt` | Refreshed the frozen `lastmod` values in `src/lib/sitemap.ts`: 14 pages bumped to `2026-06-26` (matching the 06-25/06-26 onsite edits), 3 to `2026-06-18` (the post-launch optimization wave), the rest left at their accurate earlier dates. Re-submitted all three sitemaps (`sitemap-index.xml`, `page-sitemap.xml`, `post-sitemap.xml`) in Google Search Console to clear stale 2017-era Yoast records (the root cause of the "0 pages discovered" report, though a sitemap INDEX reading 0 is normal anyway, since the children carry the URLs). Created `public/llms.txt`. | The frozen migration `lastmod`s had drifted out of sync with real content changes, which suppresses re-crawling and makes fresh pages look old; the re-submit forces GSC to drop the stale pre-migration records. The lastmod refresh and `llms.txt` are repo changes pending the next deploy. Establishes the every-change sitemap discipline now codified in the checklists above. |
| 2026-06-26 | VideoObject demoted from page subject to nested supporting media (the "2015 date" fix) | **Root cause found:** Google was printing the embedded clip's REAL `uploadDate` (Peoria `75l7CluKwIg` = 2015-04-01, Mesa `b8wO04kmk9M` = 2015-06-01) as the page's SERP date, because `videoObject()` emitted the VideoObject as a co-equal top-level node carrying `mainEntityOfPage` (which literally declares the video the page's main entity). **Fix (one shared helper, covers all 9 video city pages + 11 blog posts):** (1) removed `mainEntityOfPage` from `videoObject()` in `src/lib/videos.ts`; (2) `Layout.astro` now nests the VideoObject inside the page's primary node (`WebPage` for city pages, `BlogPosting` for posts) as `video`, deep-cloning `structuredData` so the cached JSON imports are not mutated, with a standalone fallback if no primary node exists. Verified in dist: no standalone VideoObject node, video nested under the page node, no `mainEntityOfPage`, modern WebPage `dateModified` (2026-06-12) is the dominant page date. **Also:** bumped `modifiedDate` on the 30 genuinely-edited blog posts (the 06-25 internal-linking set, identified via `git diff --diff-filter=M`) to `2026-06-26`, left the 2 untouched posts alone (no blanket bump). Self-corrects post-sitemap `lastmod`, Article `dateModified`, and the visible "Updated" line. | The stale SERP date was suppressing CTR across the whole city-page tier (the home page ranks top-7 on money terms at 0.5 to 1% CTR vs an expected 5 to 15%). ⚠️ **Do NOT revert:** never re-add `mainEntityOfPage` to the video, never emit the VideoObject as a standalone top-level node, never fake `uploadDate` (the values are real). After deploy, validate the LIVE URLs in Google Rich Results Test, then request re-indexing in GSC. Pending the next deploy. |
